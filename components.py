            ...
            # LLMからの回答の場合
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        if "main_page_number" in message["content"] and os.path.splitext(message['content']['main_file_path'])[1].lower() == '.pdf':       
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                # 参照元ドキュメントのページ番号が取得できた場合にのみ、ページ番号を表示
                                if "page_number" in sub_choice and os.path.splitext(sub_choice['source'])[1].lower() == '.pdf':
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        if "page" in llm_response["context"][0].metadata and os.path.splitext(main_file_path)[1].lower() == '.pdf':
            # ページ番号を取得
        ...
        for document in llm_response["context"][1:]:
            ...
            if "page" in document.metadata and os.path.splitext(sub_file_path)[1].lower() == '.pdf':
                # ページ番号を取得
        ...
        if sub_choices:
            ...
            for sub_choice in sub_choices:
                ...
                if "page_number" in sub_choice and os.path.splitext(sub_choice['source'])[1].lower() == '.pdf':
                    ...
                    st.info(f"{sub_choice['source']}(ページNo.{sub_page_number})", icon=icon)
                else:
                    # 「サブドキュメントのファイルパス」を表示
                    st.info(f"{sub_choice['source']}", icon=icon)
...
def display_contact_llm_response(llm_response):
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            # ページ番号が取得できた場合のみ、ページ番号を表示（ドキュメントによっては取得できない場合がある）
            if "page" in document.metadata and os.path.splitext(file_path)[1].lower() == '.pdf':
                # ページ番号を取得
